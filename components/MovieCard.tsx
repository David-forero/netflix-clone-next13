...
const MovieCard: React.FC<MovieCardProps> = ({ data }) => {
  ...
  return (
    <div className="group bg-zinc-900 col-span relative h-[12vw]">
      ...
      <div className="opacity-0 absolute top-0 transition duraction-200 z-10 invisible sm:visible delay-300 w-full scale-0 group-hover:scale-110 group-hover:-translate-y-[6vw] group-hover:translate-x-[2vw] group-hover:opacity-100">
        <img
          src={data.thumbnailUrl}
          alt=""
        ...
        <div className="z-10 bg-zinc-800 p-2 lg:p-4 absolute w-full transition shadow-md rounded-b-md">
          ...
          <div className="flex flex-row mt-4 gap-2 items-center">
            <p className="text-white text-[10px] lg:text-sm">{data?.duraction}</p>
          </div>

          <div className="flex flex-row mt-4 gap-2 items-center">
            <p className="text-white text-[10px] lg:text-sm">{data?.comedy}</p>
          </div>
        </div>
      </div>
    </div>
